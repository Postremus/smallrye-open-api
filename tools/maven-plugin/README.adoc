...
- `scanAnnotations` (List<String>) - Fully qualified names of annotations that explicitly include an operation. All operations have to have one of these annotations.
- `scanExcludeAnnotations` (List<String>) - Fully qualified names of annotations that explicitly exclude an operation. If configured, any operation with of these annotations is excluded.